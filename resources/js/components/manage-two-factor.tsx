...
export default function ManageTwoFactor(props: Props) {
    ...
    const prevTwoFactorEnabledRef = useRef(twoFactorEnabled);

    useEffect(() => {
        if (prevTwoFactorEnabledRef.current && !twoFactorEnabled) {
            clearTwoFactorAuthData();
        }

        prevTwoFactorEnabledRef.current = twoFactorEnabled;
                    ...
                    <div className="relative inline">
                        <Form {...disable.form()}>
                            {({ processing }) => (
                                ...
                                >
                                    {t(
                                        'settings.security.two_factor.disable_button',
                                    )}
                                </Button>
                            )}
                        </Form>
                            ...
                            >
                                {({ processing }) => (
                                    <Button type="submit" disabled={processing}>
                                        {t(
                                            'settings.security.two_factor.enable_button',
                                        )}
                                    </Button>
                                )}
                            </Form>
